...
class SpectrogramParser(AudioParser):
    def __init__(self,
                 ...
                 augmentation_conf: DictConfig = None):
        """
    ...
    def parse_audio(self, audio_path):
        ...
        # STFT
        #print(n_fft, hop_length, win_length, self.window)
    ...
    def parse_audio_whitenoise(self, audio_path, amt):
        if self.aug_conf and self.aug_conf.speed_volume_perturb:
            y = load_randomly_augmented_audio(audio_path, self.sample_rate)
        else:
            y = load_audio(audio_path)
        ...
        noise = np.random.uniform(low=amt*-1.0, high=amt*1.0, size=y.shape)

        D_noise = librosa.stft(noise, n_fft=n_fft, hop_length=hop_length,
                         win_length=win_length, window=self.window)

        real_noise = torch.FloatTensor(np.real(D_noise))
        imag_noise = torch.FloatTensor(np.imag(D_noise))

        mag_noise = torch.sqrt(real_noise ** 2 + imag_noise ** 2)


        """if self.aug_conf and self.aug_conf.spec_augment:
            spect = spec_augment(spect)"""

        return real, imag, mag_noise
...
class SpectrogramDataset(Dataset, SpectrogramParser):
    ...
    def __getitem__(self, index):
        ...
        real, imag, y = self.parse_audio(audio_path)
        amt=0.05
        noise = np.random.uniform(low=amt * -1.0, high=amt * 1.0, size=y.shape)
        stft = self.get_stft(noise)
        amt = len(y)/16000
        transcript = self.parse_transcript(transcript_path)

        return real, imag, transcript, stft, y
    ...
    def __len__(self):
        ...
        return int(self.audio_conf.size*self.size)
...
class AudioDataLoader(DataLoader):
    def __init__(self, dataset, num_workers, batch_size, worker_init_fn):
        """
        Creates a data loader for AudioDatasets.
        """
        super(AudioDataLoader, self).__init__(dataset, num_workers, batch_size, worker_init_fn)
