...
class WhisperASR(pl.LightningModule):
    def __init__(self,
        ...
        self.criterion = torch.nn.CTCLoss(blank=self.labels.index('_'), reduction='sum', zero_infinity=True)
        
        self.evaluation_decoder = self.processor.tokenizer
        self.wer = WordErrorRate(decoder=self.evaluation_decoder)
        self.cer = CharErrorRate(decoder=self.evaluation_decoder)

    def forward(self, input, input_lengths):
        # Preprocess input
        input_features = self.processor(input, sampling_rate=self.spect_cfg.sample_rate, return_tensors="pt").input_features
        # Forward pass through Whisper
        logits = self.model(input_features).logits
        return logits

    def training_step(self, batch, batch_idx):
        inputs, targets, _, input_percentages, target_sizes, _ = batch
        input_sizes = input_percentages.mul_(int(inputs.size(3))).int()
        
        outputs = self.forward(inputs, input_sizes)
        out = outputs.log_softmax(-1)

        loss_ctc = self.criterion(out, targets, input_sizes, target_sizes)

        if self.wandb:
            self.log('ctc_loss', loss_ctc, on_step=True, sync_dist=True)

        return {"loss": loss_ctc}
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.wandb:
            self.log('val_ctc_loss', loss_ctc, on_step=True, sync_dist=True)
    ...
    def get_seq_lens(self, input_length):
        return input_length  # For Whisper, sequence lengths may not need modification

    def training_epoch_end(self, train_step_outputs):
        loss = torch.mean(torch.tensor([x["loss"] for x in train_step_outputs]))
        self.logger.experiment.log({"epoch_loss_train": loss})

    def validation_epoch_end(self, val_step_outputs):
        loss = torch.mean(torch.tensor([x["loss"] for x in val_step_outputs]))
        self.logger.experiment.log({"epoch_loss_val": loss})
